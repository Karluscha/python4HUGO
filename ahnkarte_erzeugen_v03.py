...
VV_Text = "{:s} {:s}<br>* {:} in {:s}".format(VV['Vorname'],VV['Name'], VV['geb'], VV['ort'])
VV_Bild = "../ahnbilder/g{:d}.jpg".format(VV['anr'])

MV_Text = "{:s} {:s}<br>* {:} in {:s}".format(VV['Vorname'],MV['Name'], MV['geb'], MV['ort'])
MV_Bild = "../ahnbilder/g{:d}.jpg".format(MV['anr'])

VM_Text = "{:s} {:s}<br>* {:} in {:s}".format(VV['Vorname'],VM['Name'], VM['geb'], VM['ort'])
VM_Bild = "../ahnbilder/g{:d}.jpg".format(VM['anr'])

MM_Text = "{:s} {:s}<br>* {:} in {:s}".format(VV['Vorname'],MM['Name'], MM['geb'], MM['ort'])
...
V_Text = "{:s} {:s}<br>* {:} in {:s}".format(V['Vorname'],V['Name'], V['geb'], V['ort'])
V_Bild = "../ahnbilder/e{:d}.jpg".format(V['anr'])

P_Text = "{:s} {:s}<br>* {:} in {:s}".format(P['Vorname'],P['Name'], P['geb'], P['ort'])
P_Bild = "../ahnbilder/k{:d}.jpg".format(P['anr'])


M_Text = "{:s} {:s}<br>* {:} in {:s}".format(M['Vorname'],M['Name'], M['geb'], M['ort'])
...
<br>
  ...
  <div class="Grosseltern">
    ...
    <div class="box d">
        {:s}<br>
        <img src="{:s}" width="100%"html>
    </div>
 </div>
'''.format(V_Text, V_Bild, P_Text, P_Bild, M_Text, M_Bild)
...
for kid in Kids:
    k_Text = "{:s} {:s}<br>* {:}  in {:s}".format(kid['Vorname'],kid['Name'], kid['geb'], kid['ort'])
